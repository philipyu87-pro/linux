...
为什么使用 SR-IOV 直通网卡的虚拟机要把内存全部预占
----------------------------------------------

通过 VFIO 将 VF 直通给虚拟机后，设备会直接对客体内存做 DMA。VFIO 在建立 IOMMU
映射时会锁定这些页，要求它们不能被换出或按需分配。如果宿主机超分配或延迟分配内
存，设备在 DMA 时可能访问尚不存在的页，造成 IOMMU 故障甚至数据损坏。预占并锁定
全部客体内存可以在允许 DMA 之前保证所有页面都已就绪。
