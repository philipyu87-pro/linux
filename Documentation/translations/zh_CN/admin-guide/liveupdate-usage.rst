...
LUO 是一个**数据保留机制**，而不是进程迁移或实时迁移。
理解什么会被保留、什么不会被保留至关重要：

**LUO 会保留的内容：**

* 物理内存内容（如 memfd 页面中的数据）
* 支持的文件描述符类型的状态（memfd、有驱动支持的设备 FD）
* 硬件设备状态（需要驱动支持）

**LUO 不会保留的内容：**

* **进程状态**：进程必须重新启动；LUO 不保留正在运行的进程
* **页表**：虚拟地址映射不会保留；新内核会创建新的页表
* **虚拟地址**：物理页面会保留，但虚拟地址可能改变
* **进程内存（栈、堆、代码）**：只有显式保留的 FD（如 memfd）才会保留数据
* **寄存器状态、CPU 上下文**：进程在新内核中全新启动

**所需工作流程：**

1. **kexec 之前**：应用程序将关键数据保存到 memfd（或其他支持的 FD 类型）
2. **kexec 之后**：应用程序重新启动并使用 LUO API 检索保留的数据
3. 应用程序使用保留的数据重建其状态

这种设计允许在内核更新时保留应用程序数据，但要求应用程序"感知 LUO"并主动参与
保留/恢复过程。
...
1. **Scratch 区域**: 用于 kexec 加载新内核的物理连续内存
   ...
   - **用途**: 存放新内核镜像和 initrd
   - **早期启动分配**: 新内核在页分配器初始化前只使用 scratch 区域进行内存分配
   - **保护机制**: 防止覆盖需要保留的内存数据
   - **布局**: 每个 NUMA 节点一个 scratch 区域，外加一个通用区域
   - **CMA 机制**: 页分配器初始化后声明为 CMA，可在系统运行时重用

2. **保留区域**: 用于存储序列化的系统状态和需要跨 kexec 保留的内存数据

   - **FDT 存储**: KHO 扁平设备树（FDT）存储在 scratch 区域外的保留内存中
   - **数据传递**: FDT 物理地址通过 ``kimage->kho.fdt`` 传递，新内核接收为 ``kho_in.fdt_phys``
   - **访问方式**: 新内核使用 ``phys_to_virt()`` 映射 FDT 访问保留的元数据
...
* **memfd（内存文件描述符）**：已完整实现所有 ``liveupdate_file_ops`` 回调函数
* **其他类型**：框架支持 KVM、VFIO 和其他设备驱动，但需要驱动特定的实现

.. note::
   截至当前内核版本，只有 memfd 具有完整的 LUO 实现。
   其他设备类型（KVM、VFIO、IOMMU）在架构上已支持，但需要设备驱动维护者
   实现 ``liveupdate_file_ops`` 回调函数。
